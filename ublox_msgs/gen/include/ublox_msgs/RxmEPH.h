...
namespace ublox_msgs {
    struct RxmEPH {
        ...
        uint32_t svid;             // SV ID for which this ephemeris data is (Valid Range: 1 .. 32).
        uint32_t how;              // Hand-Over Word of first Subframe. This is
                                // required if data is sent to the receiver.;
                                // 0 indicates that no Ephemeris Data is following.
        
        // Start of optional block;
        std::vector<uint32_t> sf1d;          // Subframe 1 Words 3..10 (SF1D0..SF1D7)
        std::vector<uint32_t> sf2d;          // Subframe 2 Words 3..10 (SF2D0..SF2D7)
        std::vector<uint32_t> sf3d;          // Subframe 3 Words 3..10 (SF3D0..SF3D7)
